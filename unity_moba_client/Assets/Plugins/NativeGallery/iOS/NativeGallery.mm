...
+ (int)checkPermission:(BOOL)readPermission permissionFreeMode:(BOOL)permissionFreeMode
{
	...
	if( @available(iOS 8.0, *) )
	{
		...
		if( readPermission && permissionFreeMode && @available(iOS 11.0, *) )
			return 1;
...
+ (void)saveMedia:(NSString *)path albumName:(NSString *)album isImg:(BOOL)isImg permissionFreeMode:(BOOL)permissionFreeMode
{
	...
	if(@available(iOS 8.0, *) )
	{
		...
		//   used as fallback
		[self saveMediaNew:path albumName:album isImage:isImg saveToDefaultAlbum:( permissionFreeMode && @available(iOS 14.0, *) )];
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 80000
	}
	else
	{
...
+ (void)pickMedia:(int)mediaType savePath:(NSString *)mediaSavePath permissionFreeMode:(BOOL)permissionFreeMode selectionLimit:(int)selectionLimit
{
	...
	imagePickerState = 1;
	simpleMediaPickMode = permissionFreeMode && @available(iOS 11.0, *);
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 140000
	else if( @available(iOS 14.0, *) && imagePickerNew != nil )
	{
...
+ (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
	...
	else if( @available(iOS 9.1, *) && [info[UIImagePickerControllerMediaType] isEqualToString:(NSString *)kUTTypeLivePhoto] )
	{
...
+(void)picker:(PHPickerViewController *)picker didFinishPicking:(NSArray<PHPickerResult *> *)results
{
	...
	if( results != nil && [results count] > 0 )
	{
		...
		for( int i = 0; i < [results count]; i++ )
		{
			...
			else if( @available(iOS 9.1, *) && [itemProvider hasItemConformingToTypeIdentifier:(NSString *)kUTTypeLivePhoto] )
			{
				NSLog( @"Picked a live photo" );
...
+ (int)getMediaTypeFromExtension:(NSString *)extension
{
	...
	if( UTTypeConformsTo( fileUTI, kUTTypeImage ) )
		result = 1;
	else if( @available(iOS 9.1, *) && UTTypeConformsTo( fileUTI, kUTTypeLivePhoto ) )
		result = 1;
